...
export function assertWardTemplateFile(fileName: string): {
  extension: ".docx" | ".pdf";
  mimeType: string;
} {
...
export async function storeWardTemplateFile(
  fileName: string,
  buffer: Buffer
  ...
  const { extension, mimeType } = assertWardTemplateFile(fileName);
  const storageKey = `ward-templates/${randomUUID()}${extension}`;
...
export async function removeWardTemplateFile(storageKey: string): Promise<void> {
